$(document).ready(function() {
    ...
    $(".calendar #check_in_time2").datepicker({
        showOtherMonths: true,
        dateFormat: 'dd-mm-yy',
        minDate: 0,
        //for calender Css
        beforeShowDay: function (date) {
            return highlightDateBorder($("#check_in_time2").val(), date);
        },
        onSelect: function(selectedDate) {
            var date_format = selectedDate.split("-");
            var selectedDate = new Date($.datepicker.formatDate('yy-mm-dd', new Date(date_format[2], date_format[1] - 1, date_format[0])));
            selectedDate.setDate(selectedDate.getDate() + 1);
            $("#check_out_time2").datepicker("option", "minDate", selectedDate);
        },
    });

    $("#check_out_time2").click(function name(e) {
        
    });


    $("#check_out_time2").datepicker({
        showOtherMonths: true,
        dateFormat: 'dd-mm-yy',
        //for calender Css
        beforeShowDay: function (date) {
            return highlightDateBorder($("#check_out_time2").val(), date);
        },
        beforeShow: function (input, instance) {
            var date_to = $('#check_in_time2').val();
    ...
    var max_order_date = $('#max_order_date2').val();
    if (max_order_date != '') {
        $("#check_in_time2").datepicker("option", "maxDate", new Date(max_order_date));
        $("#check_out_time2").datepicker("option", "maxDate", new Date(max_order_date));
    }

    $("#check_in_time2, #check_out_time2").on("focus", function() {
        if ($(this).hasClass("error_border")) {
            $(this).removeClass("error_border");
            if ($(this).attr("name") == "check_in_time2")
    ...
    $('#search_room_submit, #filter_search_btn2').on('click', function(e) {
        ...
        var hotelCatId = $('#hotel_cat_id2').val();
        ...
        if (check_in_time == '') {
            $("#check_in_time2").addClass("error_border");
            $('#check_in_time_error_p2').text(check_in_time_cond);
            error = true;
        } else if (new_chk_in < $.datepicker.formatDate('yy-mm-dd', new Date())) {
            $("#check_in_time2").addClass("error_border");
            $('#check_in_time_error_p2').text(less_checkin_date);
            error = true;
        }
        if (check_out_time == '') {
            $("#check_out_time2").addClass("error_border");
            $('#check_out_time_error_p2').text(check_out_time_cond);
            error = true;
        } else if (new_chk_out < new_chk_in) {
            $("#check_out_time2").addClass("error_border");
            $('#check_out_time_error_p2').text(more_checkout_date);
            error = true;
        } else if (max_order_date_format < new_chk_in) {
            $("#check_in_time2").addClass("error_border");
            $('#check_in_time_error_p2').text(max_order_date_err + ' ' + max_order_date);
            error = true;
